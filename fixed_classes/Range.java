 ...
 */
 ...
 public strictfp class Range implements Serializable {
     ...
     public boolean intersects(double b0, double b1) {
         ...
         */
         // this is bug fixed code
         // as long as one of bounds is inside the range
             // then we have an intersection
             // even if it includes it still is intersection
         if (b0 >= this.lower && b0 <= this.upper) {
             return true;
         }
         if (b1 >= this.lower && b1 <= this.upper) {
             return true;
         }
         // however this objec'ts range  may be also included
         if (this.lower >= b0 && this.lower <= b1) {
             return true;
         }
         if (this.upper >= b0 && this.upper <= b1) {
             return true;
         }
         return false;
